...
<main class="community-page">
    <div class="container">
        <div class="community-hero">
            <div class="floating-el floating-el--1"></div>
            <div class="floating-el floating-el--2"></div>
            <div class="community-hero__content">
                <div class="community-hero__badge"><i class="bi bi-people-fill"></i> Cộng đồng</div>
                <h1 class="community-hero__title">Cùng nhau <span>học và chia sẻ</span></h1>
                <p class="community-hero__desc">Đặt câu hỏi, chia sẻ kinh nghiệm và kết nối với bạn học trên khắp mọi miền.</p>
                <div class="community-hero__stats">
                    <div class="community-hero__stat">
                        <span class="community-hero__stat-num" id="stat-posts">0</span>
                        <span class="community-hero__stat-label">Bài đăng</span>
                    </div>
                    <div class="community-hero__stat">
                        <span class="community-hero__stat-num" id="stat-likes">0</span>
                        <span class="community-hero__stat-label">Lượt thích</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="community-layout">
            <div class="community-main">
                <div class="post-box" id="post-form-card">
                    <div class="post-box__trigger" id="post-form-trigger">
                        <div class="post-box__avatar" id="trigger-avatar">H</div>
                        <span class="post-box__placeholder">Bạn đang nghĩ gì?</span>
                    </div>
                    <form id="post-form" class="post-box__form" style="display:none">
                        <div class="post-box__header">
                            <div class="post-box__avatar" id="form-avatar">H</div>
                            <div class="post-box__author-wrap">
                                <span class="post-box__author-label">Đăng bởi</span>
                                <input type="text" id="post-author" placeholder="Tên của bạn" readonly required>
                            </div>
                        </div>
                        <input type="text" id="post-title" class="post-box__field" placeholder="Tiêu đề bài viết" required>
                        <textarea id="post-content" class="post-box__field post-box__field--textarea" rows="3" placeholder="Bạn đang nghĩ gì?" required></textarea>
                        ...
                        <div style="margin-top:8px">
                            <label for="post-images" class="image-upload"><i class="bi bi-images"></i> Thêm ảnh</label>
                            <input type="file" id="post-images" accept="image/*" multiple style="display:none">
                            <div class="image-preview" id="image-preview"></div>
                        </div>
                        <div class="post-box__actions">
                            <button type="button" class="post-box__cancel" id="post-cancel">Huỷ</button>
                    ...
                    <div class="search-bar">
                        <span class="search-bar__icon"><i class="bi bi-search"></i></span>
                ...
                <aside class="community-sidebar">
                    <div class="sidebar-card">
                        <h3 class="sidebar-card__title"><i class="bi bi-tags-fill"></i> Thẻ tag phổ biến</h3>
                        <div class="tag-cloud" id="tag-cloud"></div>
                    </div>

                    <div class="sidebar-card">
                        <h3 class="sidebar-card__title"><i class="bi bi-shield-fill-check"></i> Nội quy</h3>
    ...
    <footer class="footer"><div class="footer__bottom"><div class="container"><p>&copy; 2026 HànNgữ. Thiết kế với <i class="bi bi-heart-fill" style="color:#ef4444"></i> cho cộng đồng học tiếng Trung.</p></div></div></footer>
...
async function loadPosts(page = 1) {
    ...
    list.innerHTML = '<div style="display:flex;flex-direction:column;gap:14px;padding:4px 0"><div class="skeleton skeleton--card" style="height:140px"></div><div class="skeleton skeleton--card" style="height:140px"></div><div class="skeleton skeleton--card" style="height:140px"></div></div>';
    ...
    list.innerHTML = posts.map((p, i) => {
        ...
        <article class="post-card" style="animation-delay:${i*0.06}s">
            <div class="post-card__header">
                <div class="post-card__avatar">${avatarLetter}</div>
                <div class="post-card__meta">
                    <span class="post-card__author">${escapeHtml(p.author || 'Ẩn danh')}</span>
                    <span class="post-card__date">${p.created_at || ''}</span>
                </div>
            </div>
            <h3 class="post-card__title">${escapeHtml(p.title)}</h3>
            <p class="post-card__content">${escapeHtml(p.content)}</p>
            ${p.images && p.images.length ? '<div class="post-card__images">' + p.images.map(img => '<img src="' + img + '" loading="lazy">').join('') + '</div>' : ''}
            ...
            <div class="post-card__actions">
                <button class="post-action ripple ${p.liked ? 'post-action--liked' : ''}" data-action="like" data-id="${p.id}">
                    <span class="post-action__icon">${p.liked ? '<i class="bi bi-heart-fill"></i>' : '<i class="bi bi-heart"></i>'}</span>
                    <span>${p.likes || 0}</span>
                </button>
                <button class="post-action ripple" data-action="comments" data-id="${p.id}">
                    <span class="post-action__icon"><i class="bi bi-chat"></i></span>
                    <span class="cmt-count-${p.id}">${p.comment_count || 0}</span>
                </button>
            </div>
    ...
    list.querySelectorAll('.post-action').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (action === 'like') {
                ...
                const isLiked = btn.classList.contains('post-action--liked');
                // Optimistic update
                if (icon) icon.innerHTML = isLiked ? '<i class="bi bi-heart"></i>' : '<i class="bi bi-heart-fill"></i>';
                btn.classList.toggle('post-action--liked');
                if (count) count.textContent = Math.max(0, (parseInt(count.textContent) || 0) + (isLiked ? -1 : 1));
                const likesStat = document.getElementById('stat-likes');
                if (likesStat) {
                    const cur = parseInt(likesStat.textContent) || 0;
                    likesStat.textContent = Math.max(0, cur + (isLiked ? -1 : 1));
                }
                const res = await fetchAPI('like_post', { id: id, user_id: currentUserId });
                if (!res) {
                    // Revert on failure
                    if (icon) icon.innerHTML = isLiked ? '<i class="bi bi-heart-fill"></i>' : '<i class="bi bi-heart"></i>';
                    btn.classList.toggle('post-action--liked');
                    if (count) count.textContent = Math.max(0, (parseInt(count.textContent) || 0) + (isLiked ? 1 : -1));
                    if (likesStat) {
                        const cur = parseInt(likesStat.textContent) || 0;
    ...
    // Image lightbox
    list.querySelectorAll('.post-card__images img').forEach(img => {
        img.addEventListener('click', function() {
            ...
            big.src = this.src;
            big.style.cssText = 'max-width:90%;max-height:90%;border-radius:8px;object-fit:contain';
    ...
    list.querySelectorAll('.comment-submit').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            input.value = '';
            loadComments(postId);
            const cmtCount = document.querySelector('.cmt-count-' + postId);
            if (cmtCount) cmtCount.textContent = (parseInt(cmtCount.textContent) || 0) + 1;
        ...
        btn.addEventListener('click', async function() {
            ...
            showConfirm('Xoá bình luận này?', 'Xác nhận').then(async function(r) {
                if (r) {
                    ...
                    loadComments(postId);
                    const cmtCount = document.querySelector('.cmt-count-' + postId);
                    if (cmtCount) cmtCount.textContent = Math.max(0, (parseInt(cmtCount.textContent) || 0) - 1);
                }
            });
        });
    });
...
document.getElementById('post-images')?.addEventListener('change', function() {
    ...
    for (const file of this.files) {
        ...
        reader.onload = function(e) {
            ...
            img.style.cssText = 'width:80px;height:80px;object-fit:cover;border-radius:8px;border:1px solid var(--gray-light)';
...
document.getElementById('post-form').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch('api.php?action=add_post', { method: 'POST', body: formData });
