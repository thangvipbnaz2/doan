...
<head>
    ...
    <style>
        ...
        [data-theme="dark"] .dict-page { background: linear-gradient(180deg, #0f172a, #1e293b); }
        ...
        [data-theme="dark"] .search-box input:focus { border-color: var(--teal); }
        [data-theme="dark"] .filter-tab { background: #1e293b; border-color: rgba(255,255,255,.08); color: #94a3b8; }
        [data-theme="dark"] .dict-card { background: #1e293b; }
        [data-theme="dark"] .dict-card__hanzi { color: #f1f5f9; }
        [data-theme="dark"] .dict-card__meaning { color: #94a3b8; }
        [data-theme="dark"] .dict-card__meta { color: #64748b; }
        [data-theme="dark"] .dict-detail { background: rgba(0,0,0,0.7); }
        [data-theme="dark"] .dict-detail__content { background: #1e293b; }
        [data-theme="dark"] .dict-detail__close { background: rgba(255,255,255,.08); color: #94a3b8; }
        [data-theme="dark"] .dict-detail__hanzi { color: #f1f5f9; }
        [data-theme="dark"] .dict-detail__meaning { color: #f1f5f9; border-bottom-color: rgba(255,255,255,.08); }
        [data-theme="dark"] .dict-detail__item { background: rgba(13,148,136,.1); }
        [data-theme="dark"] .dict-detail__item span { color: #f1f5f9; }
        [data-theme="dark"] .dict-detail__example { background: #0f172a; }
        [data-theme="dark"] .dict-detail__example p { color: #94a3b8; }
        [data-theme="dark"] .dict-info { color: #64748b !important; }
    </style>
</head>
<body>
...
<main class="dict-page">
    ...
    async function saveWord() {
        ...
        try {
            const res = await fetch('api.php?action=save_to_notebook', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ vocab_id: currentWord.id, user_id: userId })
            });
