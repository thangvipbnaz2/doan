...
if ($action === 'submit_review') {
    ...
    if ($known) {
        ...
        else $interval = max(1, round($interval));
    } else {
        $newConsecutive = 0;
...
if ($action === 'evaluate_handwriting') {
    ...
    foreach ($coverage as $pct) {
        if ($pct >= 15 && $pct <= 50) $totalScore += 100;
        else if ($pct >= 5 && $pct < 15) $totalScore += 60;
        else if ($pct > 50 && $pct <= 70) $totalScore += 50;
        else $totalScore += 10;
    }
...
function commerceCsrfOrFail(array $data): void {
    if (!validateCsrfToken($data['csrf_token'] ?? '')) throw new RuntimeException('Token bảo mật không hợp lệ.');
}

if ($action === 'get_courses') {
    $includeDraft = !empty($_SESSION['user_id']) && (function() use ($conn) { $s=$conn->prepare('SELECT role FROM users WHERE id=?'); $s->execute([$_SESSION['user_id']]); return $s->fetchColumn()==='admin'; })();
    $sql = 'SELECT c.*, (SELECT COUNT(*) FROM course_lessons cl WHERE cl.course_id=c.id) lesson_count FROM courses c';
    if (!$includeDraft) $sql .= ' WHERE c.is_published=1';
    $sql .= ' ORDER BY c.created_at DESC';
    echo json_encode(['success'=>true, 'courses'=>$conn->query($sql)->fetchAll()]); exit;
}
...
if ($action === 'get_course') {
    ...
    $stmt->execute([$slug]); $course=$stmt->fetch();
    if (!$course) { echo json_encode(['success'=>false,'message'=>'Không tìm thấy khóa học.']); exit; }
    ...
    $stmt->execute([$course['id']]); $course['lessons']=$stmt->fetchAll();
    $course['enrolled']=false;
    if (!empty($_SESSION['user_id'])) { $s=$conn->prepare('SELECT id FROM enrollments WHERE user_id=? AND course_id=?'); $s->execute([$_SESSION['user_id'],$course['id']]); $course['enrolled']=(bool)$s->fetchColumn(); }
    echo json_encode(['success'=>true,'course'=>$course]); exit;
}

if ($action === 'create_order') {
    try {
        $data=commerceInput(); commerceCsrfOrFail($data); $userId=commerceRequireLogin(); $courseId=(int)($data['course_id'] ?? 0);
        $stmt=$conn->prepare('SELECT * FROM courses WHERE id=? AND is_published=1'); $stmt->execute([$courseId]); $course=$stmt->fetch();
        if (!$course) throw new RuntimeException('Khóa học không khả dụng.');
        $owned=$conn->prepare('SELECT id FROM enrollments WHERE user_id=? AND course_id=?'); $owned->execute([$userId,$courseId]);
        if ($owned->fetchColumn()) throw new RuntimeException('Bạn đã sở hữu khóa học này.');
        $old=$conn->prepare("SELECT id FROM orders WHERE user_id=? AND course_id=? AND status='pending' AND expires_at > NOW() ORDER BY id DESC LIMIT 1"); $old->execute([$userId,$courseId]); $orderId=$old->fetchColumn();
        if (!$orderId) { $code=commerceOrderCode(); $new=$conn->prepare("INSERT INTO orders (order_code,user_id,course_id,amount,status,expires_at) VALUES (?,?,?,?, 'pending', DATE_ADD(NOW(), INTERVAL 30 MINUTE))"); $new->execute([$code,$userId,$courseId,$course['price']]); $orderId=(int)$conn->lastInsertId(); }
        $order=commerceOrderDetails($conn,(int)$orderId); $order['qr_url']=commerceQrUrl($order['order_code'],(float)$order['amount']);
...
if ($action === 'get_order') {
    try { $userId=commerceRequireLogin(); $order=commerceOrderDetails($conn,(int)($_GET['id']??0)); if ($order['user_id']!=$userId) commerceRequireAdmin($conn); $order['qr_url']=$order['status']==='pending'?commerceQrUrl($order['order_code'],(float)$order['amount']):''; echo json_encode(['success'=>true,'order'=>$order]); }
    catch (Throwable $e) { echo json_encode(['success'=>false,'message'=>$e->getMessage()]); } exit;
}

if ($action === 'get_my_orders') {
    try { $userId=commerceRequireLogin(); $s=$conn->prepare('SELECT o.*,c.title course_title,c.slug course_slug,i.invoice_number FROM orders o JOIN courses c ON c.id=o.course_id LEFT JOIN invoices i ON i.order_id=o.id WHERE o.user_id=? ORDER BY o.created_at DESC'); $s->execute([$userId]); echo json_encode(['success'=>true,'orders'=>$s->fetchAll()]); }
    catch(Throwable $e){ echo json_encode(['success'=>false,'message'=>$e->getMessage()]); } exit;
}

if ($action === 'admin_save_course') {
    try { $data=commerceInput(); commerceCsrfOrFail($data); commerceRequireAdmin($conn); $id=(int)($data['id']??0); $title=trim($data['title']??''); $slug=strtolower(trim($data['slug']??'')); $slug=preg_replace('/[^a-z0-9-]+/','-',$slug); $price=max(0,(int)($data['price']??0)); if (!$title||!$slug) throw new RuntimeException('Nhập tên và slug không dấu của khóa học.');
        if($id){$s=$conn->prepare('UPDATE courses SET title=?,slug=?,short_description=?,description=?,price=?,thumbnail=?,hsk_level=?,is_published=? WHERE id=?');$s->execute([$title,$slug,trim($data['short_description']??''),trim($data['description']??''),$price,trim($data['thumbnail']??''),(int)($data['hsk_level']??0)?:null,!empty($data['is_published'])?1:0,$id]);}
        else{$s=$conn->prepare('INSERT INTO courses (title,slug,short_description,description,price,thumbnail,hsk_level,is_published) VALUES (?,?,?,?,?,?,?,?)');$s->execute([$title,$slug,trim($data['short_description']??''),trim($data['description']??''),$price,trim($data['thumbnail']??''),(int)($data['hsk_level']??0)?:null,!empty($data['is_published'])?1:0]);$id=(int)$conn->lastInsertId();}
        echo json_encode(['success'=>true,'id'=>$id]);
    } catch(Throwable $e){echo json_encode(['success'=>false,'message'=>$e->getMessage()]);} exit;
}

if ($action === 'admin_set_course_lessons') {
    try { $data=commerceInput(); commerceCsrfOrFail($data); commerceRequireAdmin($conn); $courseId=(int)($data['course_id']??0); $lessonIds=$data['lesson_ids']??[]; if(!is_array($lessonIds)) $lessonIds=[];
        $conn->beginTransaction(); $conn->prepare('DELETE FROM course_lessons WHERE course_id=?')->execute([$courseId]); $s=$conn->prepare('INSERT INTO course_lessons (course_id,lesson_id,sort_order) VALUES (?,?,?)'); foreach(array_values(array_unique(array_map('intval',$lessonIds))) as $i=>$lessonId) if($lessonId>0) $s->execute([$courseId,$lessonId,$i+1]); $conn->commit(); echo json_encode(['success'=>true]);
    } catch(Throwable $e){if($conn->inTransaction())$conn->rollBack();echo json_encode(['success'=>false,'message'=>$e->getMessage()]);} exit;
}

if ($action === 'admin_list_orders') {
    try { commerceRequireAdmin($conn); $status=$_GET['status']??''; $sql='SELECT o.*,u.username,u.email,c.title course_title,i.invoice_number FROM orders o JOIN users u ON u.id=o.user_id JOIN courses c ON c.id=o.course_id LEFT JOIN invoices i ON i.order_id=o.id'; $args=[]; if(in_array($status,['pending','paid','expired','cancelled','refunded'],true)){$sql.=' WHERE o.status=?';$args[]=$status;} $sql.=' ORDER BY o.created_at DESC LIMIT 300';$s=$conn->prepare($sql);$s->execute($args);echo json_encode(['success'=>true,'orders'=>$s->fetchAll()]); }
    catch(Throwable $e){echo json_encode(['success'=>false,'message'=>$e->getMessage()]);} exit;
}

if ($action === 'admin_confirm_payment') {
    try { $data=commerceInput(); commerceCsrfOrFail($data); $adminId=commerceRequireAdmin($conn); $order=commerceCompletePayment($conn,(int)($data['order_id']??0),trim($data['transaction_id']??''),'admin_confirm',json_encode(['note'=>trim($data['note']??''), 'by'=>$adminId],JSON_UNESCAPED_UNICODE),$adminId); $sent=commerceSendInvoiceEmail($conn,$order); echo json_encode(['success'=>true,'email_sent'=>$sent,'invoice_number'=>$order['invoice_number']]); }
    catch(Throwable $e){echo json_encode(['success'=>false,'message'=>$e->getMessage()]);} exit;
}
...
【Luyện Tập】<câu hỏi tiếng Trung để học viên trả lời>';
    ...
    if (!$resp) { echo json_encode(['reply' => 'Lỗi kết nối Gemini']); exit; }
...
- Dựa vào dữ liệu từ website HànNgữ được cung cấp bên dưới để trả lời chính xác';
    ...
    if (!$resp) { echo json_encode(['reply' => 'Xin lỗi, tôi đang gặp sự cố kết nối.']); exit; }
    $json = json_decode($resp, true);
