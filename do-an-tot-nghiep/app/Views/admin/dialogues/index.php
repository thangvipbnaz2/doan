...
<head>
    ...
    <title>Quản lý hội thoại | HànNgữ Admin</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .filters select{padding:8px 12px;border:2px solid var(--gray-light);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:.85rem;background:#fafbfc;color:var(--dark)}
...
<div class="admin-layout">
    ...
    <main class="admin-content">
        <div class="admin-header">
            <h1>Hội thoại</h1>
            <a href="admin_mvc.php?action=dialogues_create" class="btn btn--primary">+ Thêm hội thoại</a>
        </div>

        <div class="admin-card">
            <div class="filters">
                <form method="GET">
                    <input type="hidden" name="action" value="dialogues">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;gap:4px">
                                <a href="admin_mvc.php?action=dialogues_edit&id=<?= (int)$d['id'] ?>" class="btn btn--small btn--primary">Sửa</a>
                                <a href="admin_mvc.php?action=dialogues_delete&id=<?= (int)$d['id'] ?>" class="btn btn--small btn--danger" onclick="return confirm('Xóa hội thoại này?')">Xóa</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
            ...
            <div class="pagination">
                ...
                <a href="admin_mvc.php?action=dialogues&page=<?= $p ?>&lesson_id=<?= $lessonId ?>" class="page-link <?= $p === $page ? 'active' : '' ?>"><?= $p ?></a>
                <?php endfor; ?>
            </div>
            <?php endif; ?>
        </div>
