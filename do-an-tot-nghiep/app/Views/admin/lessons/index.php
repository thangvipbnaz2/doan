...
<head>
    ...
    <title>Quản lý bài học | HànNgữ Admin</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        @media(max-width:640px){.admin-sidebar{display:none}.admin-content{margin-left:0}.admin-table{font-size:.82rem}.filter-tabs{overflow-x:auto;flex-wrap:nowrap}}
    </style>
</head>
<body>
<div class="admin-layout">
    <nav class="admin-sidebar">
        ...
        <a href="admin_mvc.php?action=lessons" class="admin-nav-item admin-nav-item--active"><span class="icon">📖</span> Bài học</a>
    ...
    <main class="admin-content">
        <div class="admin-header">
            <h1>Bài học</h1>
            <a href="admin_mvc.php?action=lesson_create" class="btn btn--primary">+ Thêm bài học</a>
        </div>

        <div class="filter-tabs">
            <a href="admin_mvc.php?action=lessons" class="filter-tab <?= !$level ? 'active' : '' ?>">Tất cả</a>
            <?php for ($i = 1; $i <= 6; $i++): ?>
            <a href="admin_mvc.php?action=lessons&level=<?= $i ?>" class="filter-tab <?= $level === $i ? 'active' : '' ?>">HSK <?= $i ?></a>
            <?php endfor; ?>
        </div>
        ...
        <div class="admin-card">
            <table class="admin-table">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <a href="admin_mvc.php?action=lesson_edit&id=<?= (int)$l['id'] ?>" class="btn btn--small btn--primary">Sửa</a>
                            <a href="admin_mvc.php?action=lesson_delete&id=<?= (int)$l['id'] ?>" class="btn btn--small btn--danger" onclick="return confirm('Xóa bài học này?')">Xóa</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <div class="pagination">
                ...
                <a href="admin_mvc.php?action=lessons&page=<?= $p ?>&level=<?= $level ?>" class="page-link <?= $p === $page ? 'active' : '' ?>"><?= $p ?></a>
                <?php endfor; ?>
            </div>
            <?php endif; ?>
        </div>
