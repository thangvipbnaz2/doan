...
<head>
    ...
    <title><?= isset($speakingItem) && $speakingItem ? 'Chỉnh sửa bài nói' : 'Thêm bài nói' ?> | HànNgữ Admin</title>
...
<div class="admin-layout">
    <nav class="admin-sidebar">
        ...
        <a href="admin_mvc.php?action=speaking" class="admin-nav-item admin-nav-item--active"><span class="icon">🎤</span> Nói</a>
    ...
    <main class="admin-content">
        <div class="admin-header">
            <h1><?= isset($speakingItem) && $speakingItem ? 'Chỉnh sửa bài nói' : 'Thêm bài nói mới' ?></h1>
            <a href="admin_mvc.php?action=speaking" class="btn btn--outline">← Quay lại</a>
        </div>

        <div class="form-card">
            <form method="POST" action="admin_mvc.php?action=<?= isset($speakingItem) && $speakingItem ? 'speaking_update&id=' . (int)$speakingItem['id'] : 'speaking_save' ?>">
                <div class="fg">
                    <label>Bài học</label>
                    <select name="lesson_id" required>
                        <option value="">Chọn bài học</option>
                        <?php foreach ($allLessons as $ls): ?>
                        <option value="<?= (int)$ls['id'] ?>" <?= isset($speakingItem) && (int)$speakingItem['lesson_id'] === (int)$ls['id'] ? 'selected' : '' ?>>
                ...
                <div class="fg">
                    ...
                    <textarea name="instruction" rows="3" placeholder="Hãy đọc to đoạn văn sau..."><?= isset($speakingItem) ? escape($speakingItem['instruction']) : '' ?></textarea>
                </div>
                <div class="fg">
                    <label>Văn bản mục tiêu (chữ Hán)</label>
                    <textarea name="target_text" rows="4" style="font-family:var(--font-hanzi);font-size:1.1rem"><?= isset($speakingItem) ? escape($speakingItem['target_text']) : '' ?></textarea>
                </div>
                <div class="fg">
                    <label>Pinyin mục tiêu</label>
                    <input type="text" name="target_pinyin" value="<?= isset($speakingItem) ? escape($speakingItem['target_pinyin']) : '' ?>" placeholder="wǒ shì xuésheng">
                </div>
                <div class="fg">
                    <label>Audio tham chiếu (URL)</label>
                    <input type="url" name="audio_reference" value="<?= isset($speakingItem) ? escape($speakingItem['audio_reference']) : '' ?>" placeholder="https://example.com/reference.mp3">
                </div>
                <div class="fg">
                    <label>Thứ tự</label>
                    <input type="number" name="sort_order" value="<?= isset($speakingItem) ? (int)$speakingItem['sort_order'] : 0 ?>" min="0">
                </div>
                <div style="display:flex;gap:12px;margin-top:24px">
                    <button type="submit" class="btn btn--primary" style="flex:1">💾 <?= isset($speakingItem) && $speakingItem ? 'Cập nhật' : 'Tạo mới' ?></button>
                    <a href="admin_mvc.php?action=speaking" class="btn btn--outline">Hủy</a>
                </div>
            </form>
        </div>
    </main>
