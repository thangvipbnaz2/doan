...
<head>
    ...
    <title>Quản lý người dùng | HànNgữ Admin</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .filters{display:flex;gap:10px;align-items:center;padding:20px 24px;border-bottom:1px solid var(--gray-light);flex-wrap:wrap}
        .filters input[type=text]{flex:1;padding:10px 14px;border:2px solid var(--gray-light);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:.85rem;background:#fafbfc;color:var(--dark);min-width:200px}
        .filters input:focus{outline:none;border-color:var(--teal)}
        ...
        .role-select{padding:6px 10px;border:1px solid var(--gray-light);border-radius:6px;font-family:var(--font-sans);font-size:.82rem;background:#fff}
        .role-admin{background:#dbeafe;color:#2563eb;font-weight:600}
...
<div class="admin-layout">
    <nav class="admin-sidebar">
        ...
        <a href="admin_mvc.php?action=users" class="admin-nav-item admin-nav-item--active"><span class="icon">👥</span> Người dùng</a>
    ...
    <main class="admin-content">
        ...
        <div class="admin-card">
            ...
            <table class="admin-table">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="admin_mvc.php?action=user_update_role">
                        ...
                        <td>
                            <a href="admin_mvc.php?action=user_delete&id=<?= (int)$u['id'] ?>" class="btn btn--small btn--danger" onclick="return confirm('Xóa người dùng <?= escape($u['username']) ?>?')">Xóa</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <div class="pagination">
                ...
                <a href="admin_mvc.php?action=users&page=<?= $p ?>&search=<?= escape($search) ?>" class="page-link <?= $p === $page ? 'active' : '' ?>"><?= $p ?></a>
                <?php endfor; ?>
            </div>
            <?php endif; ?>
        </div>
