...
<head>
    ...
    <style>
        ...
        [data-theme="dark"] .auth-page { background: linear-gradient(160deg, #0f172a, #1e293b); }
        [data-theme="dark"] .auth-card { background: #1e293b; }
        [data-theme="dark"] .auth-card__title { color: #f1f5f9; }
        [data-theme="dark"] .auth-card__desc { color: #94a3b8; }
        [data-theme="dark"] .fg label { color: #94a3b8; }
        [data-theme="dark"] .fg input { background: #0f172a; border-color: rgba(255,255,255,.08); color: #f1f5f9; }
        [data-theme="dark"] .fg input:focus { border-color: var(--teal); }
        [data-theme="dark"] .password-toggle { color: #64748b; }
        [data-theme="dark"] .auth-footer { color: #64748b; }
        [data-theme="dark"] .auth-footer a { color: var(--teal); }
        [data-theme="dark"] .auth-card label[style*="color"] { color: #94a3b8 !important; }
        [data-theme="dark"] .auth-card input[type="checkbox"] + span { color: #64748b !important; }
        [data-theme="dark"] .auth-card a[style*="color"] { color: #64748b !important; }
    </style>
...
<body>
            ...
            <form id="login-form">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;">
                    <label style="font-size:.85rem;color:var(--gray);display:flex;align-items:center;gap:6px;cursor:pointer;">
                        ...
                        <span style="font-size:.7rem;color:var(--gray);margin-left:4px;">(1 ngày)</span>
                    </label>
    ...
    fetch('auth.php?action=check').then(r=>r.json()).then(d=>{if(d.logged_in){const a=['admin','super_admin','content_creator','moderator'];window.location.href=a.includes(d.user?.role)?'admin.php':'index.php';}}).catch(()=>{});
    ...
    document.getElementById('login-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const adminRoles = ['admin','super_admin','content_creator','moderator'];
                setTimeout(() => { window.location.href = adminRoles.includes(result.user.role) ? 'admin.php' : 'index.php'; }, 1000);
            } else {
